  ...
  return (
    <Popover.Root open={isOpen} onOpenChange={setIsOpen}>
      <Popover.Trigger asChild>
        <button
          className={cn(
            "flex items-center gap-2 rounded-full border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-1.5 text-xs font-medium hover:bg-gray-50 dark:hover:bg-gray-700",
            !selectedDate && "text-gray-400 dark:text-gray-500"
          )}
        >
          <CalendarIcon className="h-3 w-3 text-gray-400 dark:text-gray-500" />
          {selectedDate ? format(selectedDate, "MM/dd/yyyy hh:mm a") : placeholder}
        </button>
      </Popover.Trigger>
      <Popover.Portal>
        <Popover.Content
          className="z-50 w-auto rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-0 shadow-lg"
          sideOffset={5}
        >
          <div className="p-3">
            ...
            <div className="grid grid-cols-7 gap-1 text-center text-xs">
              {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((day) => (
                <div key={day} className="p-1 font-medium text-gray-500 dark:text-gray-400">
                  {day}
                </div>
              ))}
            ...
            <div className="mt-2 border-t border-gray-200 dark:border-gray-700 pt-2">
              <div className="flex items-center gap-2 text-xs">
                <input
                  type="time"
                  defaultValue={selectedDate ? format(selectedDate, "HH:mm") : "12:00"}
                  onChange={(e) => {
                    if (selectedDate) {
                  ...
                  className="px-2 py-1 border border-gray-200 dark:border-gray-700 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
                />
                <button
                  onClick={() => {
                    const now = new Date();
